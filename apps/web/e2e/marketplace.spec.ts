...
test('completes a full purchase: add → cart → checkout → confirm', async ({
  page,
}) => {
  await page.goto('/');
  await page
...
test('tracks an order: place → confirm → track shows route + ETA', async ({
  page,
}) => {
  await page.goto('/');
  await page
...
test('checkout surfaces a recoverable error when the request fails', async ({
  page,
}) => {
  await page.goto('/');
  await page
