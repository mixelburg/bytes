...
import { productQueryOptions, type ProductDetail, type ListItem } from '../data/queries';
...
function Grid({ children }: { children: React.ReactNode }) {
  return (
    <Box sx={{ mt: 1.75, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '20px 16px' }}>
      {children}
    </Box>
  );
}
...
export default function SavedScreen() {
  ...
  if (ids.length === 0) {
    return (
      <CenterState glyph="♡" title="Nothing saved yet" body="Tap the heart on a product to keep it here for later.">
  ...
  if (allFailed) {
    return (
      <CenterState glyph="!" title="Couldn’t load saved items" body="Something went wrong. Check your connection and try again.">
        <Button variant="contained" sx={{ mt: 2.5, height: 46, px: 3 }} onClick={() => queries.forEach((q) => q.refetch())}>
          Retry
        </Button>
      </CenterState>
    );
  ...
  return (
    <Box sx={{ px: 2.5, pt: 1.5, pb: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <Mono>{ids.length} {ids.length === 1 ? 'ITEM' : 'ITEMS'}</Mono>
      </Box>
