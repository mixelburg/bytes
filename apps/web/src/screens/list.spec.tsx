...
const LIST = [
  { id: 1, title: 'Walnut Lamp', category: 'home', image: '', rating: 4.7, priceMin: 89, priceMax: 89, inStock: true },
  { id: 2, title: 'Field Backpack', category: 'outdoor', image: '', rating: 4.8, priceMin: 54, priceMax: 98, inStock: true },
  { id: 3, title: 'Wool Sweater', category: 'clothes', image: '', rating: 4.5, priceMin: 96, priceMax: 96, inStock: false },
];
...
const detailFor = (id: number) => ({
  ...
  priceMax: 89,
  variants: [
    { id: 900 + id, productId: id, options: {}, optionsLabel: '', price: 89, stock: 10, image: '' },
  ],
});
  ...
  api: {
    products: {
      $get: async ({ query }: { query: Record<string, string> }) => {
        ...
        return ok({ items: filtered.slice(0, page * limit), total: filtered.length, page, limit, hasMore: page * limit < filtered.length });
      },
      ':id': {
        $get: async ({ param }: { param: { id: string } }) => ok(detailFor(Number(param.id))),
      },
    },
    categories: { $get: async () => ok(['home', 'outdoor', 'clothes']) },
    orders: { $post: async () => ({ ok: true, status: 201, json: async () => ({ id: 'ck_test', total: 89 }) }) },
  },
}));
...
describe('Marketplace (mocked API)', () => {
  ...
  it('quick-adds a product (resolving its variant) to the cart', async () => {
    ...
    await waitFor(() =>
      expect(Object.values(store.getState().cart.items).reduce((s, e) => s + e.qty, 0)).toBe(1)
    );
  ...
  it('shows the empty state for a no-match search', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/search products/i), { target: { value: 'zzz-nope' } });
