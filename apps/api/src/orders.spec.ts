...
async function aStockedVariant(): Promise<{
  productId: number;
  variant: Variant;
}> {
...
describe('POST /orders (variant-level)', () => {
  it('places an order against a variant and decrements its stock', async () => {
    ...
    const updated = (after.variants as Variant[]).find(
      (v) => v.id === variant.id,
    )!;
  ...
  it('rejects insufficient variant stock with 409', async () => {
    ...
    const res = await post({
      items: [{ variantId: variant.id, quantity: variant.stock + 1_000_000 }],
    });
  ...
  it('rejects an unknown variant with 400', async () => {
    const res = await post({
      items: [{ variantId: 999_999_999, quantity: 1 }],
    });
  ...
  it('persists the delivery address and exposes it via GET /orders/:id', async () => {
    ...
    expect(
      order.stops.some((s: { label: string }) => s.label.includes('Berlin')),
    ).toBe(true);
