...
const seedUrl = (tag: string): string =>
  `https://picsum.photos/seed/${tag}/400/400`;
...
console.log(
  `re-seeding ${deadProducts.length} products + ${deadVariants.length} variants with picsum URLs`,
);

for (const r of deadProducts) {
  await db.product.update({
    where: { id: r.id },
    data: { image: seedUrl(`prod-${r.id}`) },
  });
}
for (const r of deadVariants) {
  await db.productVariant.update({
    where: { id: r.id },
    data: { image: seedUrl(`var-${r.id}`) },
  });
}
